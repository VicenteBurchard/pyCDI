    ...
    for year in np.unique(years):
        ...
        for month in np.unique(months):
            ...
            for m in months_ip:
                ...
                new_values = np.zeros_like(mean_ar)
                # all values below ltm assigned 1
                new_values[values < ltm_mean] = 1
                # addition of new values to pixels which were previously below LTM (consecutive)
                rl_accum = np.logical_and(rl_month > 0, values < ltm_mean)
                new_values[rl_accum] = rl_month[rl_accum] + new_values[rl_accum]
                # assign 1 to pixels which were not previously below LTM
                rl_new = np.logical_and(rl_month == 0, values < mean_ar)
                new_values[rl_new] = 1
                rl_month = new_values
...
def resample_indices(outfolder, roi_shapefile, template_file, indices=['PDI', 'TDI', 'VDI']):
    '''
    ...
    for di in indices:
        ...
        for img in img_list:
            ...
            tempfile.unlink()
    # Assign coordinates
    pdi_ds.coords['time'] = np.array(times)
    ...
    tdi_ds.coords['time'] = np.array(times)
    ...
    vdi_ds.coords['time'] = np.array(times)
